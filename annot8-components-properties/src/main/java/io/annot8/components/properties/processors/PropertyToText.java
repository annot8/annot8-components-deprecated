...
public class PropertyToText extends AbstractComponent implements Processor {
  ...
  public void configure(Context context) throws BadConfigurationException, MissingResourceException {
    ...
    settings = context.getSettings(PropertyToTextSettings.class)
        .orElseGet(PropertyToTextSettings::new);
  }
  ...
  public ProcessorResponse process(Item item) throws Annot8Exception {
    item.getProperties().getAll().entrySet().stream()
      ...
      .filter(e -> settings.getWhitelist().isEmpty() || settings.getWhitelist().contains(e.getKey())) //Key must be on whitelist, if it is set
      .forEach(e -> {
        try {
