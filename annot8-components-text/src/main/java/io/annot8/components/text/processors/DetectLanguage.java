...
/**
 * Detect the language of a Text content and add the language as an annotation
...
public class DetectLanguage extends AbstractComponent implements Processor {
  ...
  public void configure(Context context) throws BadConfigurationException, MissingResourceException {
    ...
    languageDetector = LanguageDetectorBuilder.create(detectLanguageSettings.getNgramExtractor())
  ...
  public ProcessorResponse process(Item item) {
    ...
    item.getContents(Text.class)
        ...
        .forEach(t -> {
          TextObject textObject = detectLanguageSettings.getTextObjectFactory()
              .forText(t.getData());
